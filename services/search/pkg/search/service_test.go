...
var _ = DescribeTable("ParseLocationFilters",
	...
	Entry("tokens inside a group",
		`(mediatype:image AND driveId:"storage$space" AND path:"a b") AND photo.takenDateTime>=2019-12-01`,
		`(mediatype:image) AND photo.takenDateTime>=2019-12-01`,
		`storage$space`,
		`a b`,
	),
	Entry("group that only held the tokens collapses",
		`(driveId:"storage$space" AND path:"a") AND mediatype:image`,
		`mediatype:image`,
		`storage$space`,
		`a`,
	),
