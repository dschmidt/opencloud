...
var _ = Describe("Tika", func() {
	Describe("extract", func() {
		...
		It("adds motion photo content from the legacy MicroVideo format", func() {
			fullResponse = `[
				{
					"Camera:MicroVideoVersion": "1",
					"Camera:MicroVideoPresentationTimestampUs": "500000",
					"Camera:MicroVideoOffset": "1234567"
				}
			]`
			doc, err := tika.Extract(context.TODO(), &provider.ResourceInfo{
				Type: provider.ResourceType_RESOURCE_TYPE_FILE,
			...
			Expect(motionPhoto.VideoSize).To(Equal(libregraph.PtrInt64(1234567)))
		})

		It("adds motion photo content from the current Container format", func() {
			// videoSize is the length of the container item whose semantic is MotionPhoto.
			fullResponse = `[
				{
					"Camera:MotionPhotoVersion": "1",
					"Camera:MotionPhotoPresentationTimestampUs": "500000",
					"Container:Directory[1]/Item:Semantic": "Primary",
		...
		It("skips a motion photo without a video size", func() {
			// Without a video size the facet cannot drive the range fetch, so it is not stored.
			fullResponse = `[
				{
					"Camera:MotionPhotoVersion": "1",
					"Camera:MotionPhotoPresentationTimestampUs": "500000"
